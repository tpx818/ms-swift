...
- activation_cpu_offload: Activation offloading is a memory optimization technique that offloads activation tensors saved during the forward pass to CPU memory, and reloads them back to GPU when needed during the backward pass. This feature significantly reduces GPU memory usage, allowing you to train larger models or use larger batch sizes. To enable activation offloading during training, simply add the `--activation_cpu_offload true` flag
